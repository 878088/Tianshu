...
export default {
  async fetch(访问请求) {
    ...
    if (url.pathname === '/api/proxy.json' && 访问请求.headers.get('Upgrade') === 'websocket') {
      ...
      const 转发计数 = parseInt(访问请求.headers.get('forward-count') || '0');
      
      if (转发计数 >= 3) {
        return new Response(JSON.stringify({ error: '转发次数过多' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' }
        });
      }
      ...
      if (下一级转发地址) {
        ...
        新标头.set('forward-count', (转发计数 + 1).toString());
...
async function 尝试建立TCP连接(地址, 端口, 地址类型) {
  ...
  try {
    ...
    await Promise.race([
      TCP接口.opened,
      new Promise((_, reject) => setTimeout(() => reject(new Error('TCP连接超时')), 超时时间))
    ]);
    return TCP接口;
  } catch (error) {
  ...
  if (启用反代功能) {
    try {
      ...
      await Promise.race([
        TCP接口.opened,
        new Promise((_, reject) => setTimeout(() => reject(new Error('反代连接超时')), 超时时间))
      ]);
